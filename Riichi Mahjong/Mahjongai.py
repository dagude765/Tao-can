...
#ai策略4（针对混色子）
    def dapai4(self,_pai):
        ...
        # elif _pai>10:
            # self.wan.append(_pai)
        print("听标志"+str(self.tingflag))
        print("摸牌")
        print(self.hand+[_pai])
        if self.tingflag == True:
            self.exp, self.rongflag = self.rong2(self.hand+[_pai])
            print(self.rongflag)
            if self.rongflag == True:
                self.hand.append(_pai)
                tmp = 0
                ...
                #self.drop.append(_pai)
                return _pai
        else:
            tmp, self.tingflag = self.keyiting(self.hand+[_pai])
            self.hand.append(_pai)
            if _pai>40:
                self.zi.append(_pai)
            elif _pai>30:
    ...
    def keyiting(self,_list):
        rongflag = False
        buchong = list(set(_list))
        for pai in buchong:
            i = _list.index(pai)
            newlist = _list[0:i]+_list[i+1:]
            for mopai in list(range(31,40))+list(range(41,48)):
                exp, rongflag = self.rong2(newlist+[mopai])
                if rongflag == True:
                    print("胡牌")
                    print(sorted(newlist+[mopai]))
                    return (pai,True)
        return (0,False)
        ...
        if _list[0] // 10 ==4:
            ...
            if test%3 ==0:
                ...
                for tou in [ptou,ptou+3,ptou+6]:
                    ...
                    if num>=2:
                        i = _list.index(tou)
                        (exp1,flag1) = self.explain(sorted(_list[:i]+_list[i+2:]))

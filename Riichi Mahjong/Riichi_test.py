...
def genJiesuan(screen, _g, TAG):
    if TAG==True:
        ...
        font = pygame.font.Font('simsun.ttc', 24)
        screen.blit(font.render(u'电脑胡了', True, (0,0,0)),(360,50))
...
def genRightx(screen, _x, _hand):
    if 0<_x and _x<tilesize[0]*len(_hand):
        return (1, True)
    elif (_x-8)//tilesize[0] -len(_hand)==0:
        return (2, True)
    else:
        return (0, False)
        ...
        for event in pygame.event.get():
            ...
            if event.type == MOUSEBUTTONDOWN:
                ...
                if TAG == True or TAG == 2 or TAG == 3:
                    ...
                    x -=left
                    y -=up
                    choose, pressflag = genRightx(screen, x, _g.user.hand)
                    if 0<y and y<tilesize[1] and pressflag == True:
                        if choose == 1:
                            if _g.user.riichi>0:
                                pass
                        ...
                        if _pai == 0: TAG == 2
                            ...
                            _dropai = _g.ai.dapai4(_pai)
                            print(_g.ai.hand)
                            if _dropai == 0:
                                TAG = 3
                            else:
                                _g.ai.drop.append(_dropai)
                                _g.ai.hand.sort()
                                _pai = _g.serve()
                                print(_g.user.drop)
                                print(_g.user.riichi)
                        else:
                            GANGTAG = 0
                        if _pai==0: TAG = 2
            ...
            genDora(screen,_g)
            genHand(screen,_g.user.hand, _pai, TAG)
            genAihand(screen,_g.ai.hand, _pai, TAG) #xin
            genCPG(screen,_g.user)
            genDrop(screen,_g.user.drop)
            genAiDrop(screen,_g.ai.drop)
...
'''            
            screen.fill((255, 255, 255))
            genDora(screen,_g)
            genAihand(screen,_g.ai.hand, _pai)            
'''

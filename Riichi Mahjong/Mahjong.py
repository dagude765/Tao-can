...
class game():
    ...
    def __init__(self):
        ...
        self.create()
        self.user = player()
        self.ai = Mahjongai.aiplayer()#xin
        self.quan = 0
        self.oya = -1
        self.xun = 0
        self.benchang = 0
    ...
    def newset(self):
        ...
        #ai
        self.ai.__init__()
        self.ai.hand, self.yama = self.yama[-13:],self.yama[:-13]
        self.ai.hand.sort()
        for tmp in self.ai.hand:
            if tmp<20:
                self.ai.wan.append(tmp)
            elif tmp<30:
                self.ai.bing.append(tmp)
            elif tmp<40:
                self.ai.tiao.append(tmp)
            else:
                self.ai.zi.append(tmp)

    def serve(self):
        #if len(self.yama)<=14 or self.xun >=30: return 0
        if len(self.yama)<=6: return 0
        else:
            pai = self.yama.pop()
    ...
    def jiesuan(self, _pai):
        ...
        if self.yi!=[0,0]:
            if self.yi[1]!=0:
                ...
                if self.yi[0] ==0: self.dedian = -8000
                elif self.jbd < 2000:
                    if self.user.position == self.oya: self.dedian = math.ceil(self.jbd*6/100) *100
                    else: self.dedian = math.ceil(self.jbd*4/100)*100
                else:
                    if self.yi[0]<=5: self.dedian =8000
                    elif self.yi[0] <=7: self.dedian = 12000
                    elif self.yi[0] <=10: self.dedian = 16000
                    elif self.yi[0] <=12: self.dedian = 24000
                    elif self.yi[0]>=13:self.dedian = 32000
...
class player():
    ...
    def calcfu(self,quan,oya):
        ...
        if flag == False:
            ...
            for kezi in self.peng:
                tmp = kezi[0]
                if self.isyao(tmp): res +=4
                else: res +=2
            # gang
            for a in self.agang:
                tmp = a[0]
                if self.isyao(tmp): res +=32
                else: res +=16
            for a in self.mgang:
                tmp = a[0]
                if self.isyao(tmp): res +=16
                else: res +=8
            for k,v in exp.items():
                ...
                if tmp1 == 4:
                    ...
                    if tmp2>=5:
                        self.fu[k] +=2
                for zuhe in v:
                    if len(zuhe)==2: continue
                    elif zuhe[1]==zuhe[0]:
                        if self.zimo>0 or self.rongpai!=zuhe[0]:
                            if self.isyao(zuhe[0]): self.fu[k] +=8
                            else: self.fu[k] +=4
                        else:
                            tag = 0
                            if self.rongpai == zuhe[0]:
                                for tmp in v:
                                    if self.rongpai in tmp and tmp!=zuhe:
                                        tag = 1
                            if tag ==0:
                                if self.isyao(zuhe[0]) : self.fu[k] +=4
                                else: self.fu[k] +=2
                            else:
                                if self.isyao(zuhe[0]) : self.fu[k] +=8
                                else: self.fu[k] +=4
    ...
    def calcyi(self,quan,oya):
        ...
        if flag == False: self.yi = {1:[[0],[0]]}
            ...
            for k,v in exp.items():
                ...
                quetou = []
                for tmp in v:
                    if len(tmp)==2: quetou.append(tmp)
                    elif tmp[0]==tmp[1]:
                        kezi.append(tmp)
                    else:
                        shunzi.append(tmp)
                ...
                if b>=3:
                    if self.sanse(shunzi): ###
                        if self.isclose: self.yi[k][0][0] +=2
                        else: self.yi[k][0][0] +=1
    ...
    def rong(self, _pai, quan, oya):
        ...
        if self.rongflag == False:
            ...
            if maxyi==[0,0]:
                ...
                if maxk[0] in self.fu.keys(): a = self.fu[maxk[0]]
                else: a=0
                if maxk[1] in self.fu.keys(): b = self.fu[maxk[1]]
                else: b=0
        ...
        if tmp == [0,0,0,2]:
            ...
            for l,_list in zip([a,b,c,d],[wan,bing,tiao,zi]):
                if l<8:
                    ...
                    if flag == False : return (expres,flagres)
                    else:
                        flagres = True
                        hand = hand + exp[1]
            l = max(a,b,c,d)
    ...
    def explain(self,_list):
        ...
        if _list[0] // 10 ==4:
            if test % 3 ==0:
                ...
                num = _list.count(_list[0])
                if num!=2 and num!=3: return ({},False)
                else:
    ...
    def laotou(self,kezi,quetou):
        ...
        for zuhe in kezi+quetou:
            if zuhe[0] in zipai: z +=1
            elif zuhe[0] in laotou: l +=1
            else: return 0
        if l==0: return 3
        elif z==0: return 2
        else: return 1
    ...
    def dasanyuan(self,kezi):
        if kezi[-1][0]==47 and kezi[-2][0] ==46 and kezi[-3][0]==45: return True
        else: return False
    ...
    def xiaosixi(self,kezi,quetou):
        ...
        for v in kezi+quetou:
            if v[0]//10 !=4: continue
            else: test +=[v[0]]
        test.sort()
        if test[:4] == [41,42,43,44]: return True
        elif test[1:] == [41,42,43,44]: return True
        else: return False
            ...
            if self.zimo >0:
                ...
                for v in shunzi+quetou:
                    if self.rongpai in v:
                        _find = 1
                        break
                if _find == 1: return True
                elif len(kezi) ==4: return True
                else: return False
        else:
            tmp =len(self.peng)+len(self.mgang)
            if tmp>1: return False
            elif tmp==1 and len(shunzi)>0: return False
            else:
                if self.zimo>0 or self.rongpai in quetou[0]: return True
                else: return False
    ...
    def santongke(self,kezi):
        ...
        test = max(tmp1)
        if test==3 and tmp[tmp1.index(3)]!=0: return True
        else: return False

    def xiaosanyuan(self,kezi,quetou):
        if quetou[0][0] not in [45,46,47]: return False
        else:
            if kezi[-1][0] in [45,46,47] and kezi[-2][0] in [45,46,47]: return True
            else: return False
    ...
    def yaojiu(self,kezi,shunzi,quetou):
        ...
        for v in shunzi:
            if v[0]%10 ==1 or v[2]%10==9: continue
            else: return 0
        for v in kezi+quetou:
            if v[0] in zipai: z +=1
            elif v[0] in laotou: l +=1
            else: return 0
        if z==0: return 2
        else: return 1
    ...
    def yise(self,kezi,shunzi,quetou):
        ...
        if len(shunzi)!=0:
            ...
            for v in kezi+quetou:
                if v[0]//10 == 4 : z+=1
                elif v[0]//10 == tmp: continue
                elif tmp==4: tmp = v[0]//10
                else: return 0
            if tmp == 4: return 13
            elif z==0: return 5
            else: return 2
...
class aiplayer():
    ...
    #ai策略1：随机打出
    def dapai(self,_pai):
        self.hand.append(_pai)
        random.shuffle(self.hand)
        tmp = self.hand[1]
        self.drop.append(tmp)
        self.hand.remove(tmp)
        return tmp
    #ai策略2：先打孤张字，再打字
    def dapai2(self,_pai):
        self.hand.append(_pai)
        if _pai>40:
            self.zi.append(_pai)
        elif _pai>30:
            self.tiao.append(_pai)
        elif _pai>20:
            self.bing.append(_pai)
        elif _pai>10:
            self.wan.append(_pai)
        if len(self.zi) == 0:
            random.shuffle(self.hand)
            tmp = self.hand[1]
            self.drop.append(tmp)
            self.hand.remove(tmp)
            if tmp>30:
                self.tiao.remove(tmp)
            elif tmp>20:
                self.bing.remove(tmp)
            else:
                self.wan.remove(tmp)
        else:
            for zipai in self.zi:
                num = self.zi.count(zipai)
                if num == 1:
                    tmp = zipai
                    self.drop.append(tmp)
                    self.hand.remove(tmp)
    ...
    def dapai3(self,_pai):
        ...
        if _pai>40:
            ...
            self.wan.append(_pai)
        if len(self.zi) == 0:
            self.exp, self.rongflag = self.rong2(self.hand)
            print(self.rongflag)
            if self.rongflag == True:
                tmp = 0
                return tmp
            else:
                random.shuffle(self.hand)
    ...
    def rong2(self,_hand) :
        ...
        if tmp == [0,0,0,2]:
            ...
            # else:
                # expres[1] = tmp1
                # flagres = False
            for l,_list in zip([a,b,c,d],[wan,bing,tiao,zi]):#好用
                if l<8:
                    (exp,flag) = self.explain(_list)
                    if flag == False : return (expres,flagres)
                    else:
...
def main():
    ...
    _l = [11,11,12,12,13,13,14,14,15,15,16,16,47,47]
